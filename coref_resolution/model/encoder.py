...
class Encoder(torch.nn.Module):
  def __init__(self, config, use_cache=True):
    ...
    try:
      self.tokenizer = transformers.AutoTokenizer.from_pretrained(config["encoder_name"], use_fast=False) # replace with local files
    except:
      logging.info("Did not found tokenizer, using spanbert-cased")
      self.tokenizer = transformers.AutoTokenizer.from_pretrained("SpanBERT/spanbert-base-cased", use_fast=False)
